...
`v2.0.0` public contract, and must not be imported directly by a v2.0.0-pinned
consumer. The public `gh` symbols remain stable since `2.0.0`; their current
implementation records `process` as an implementation-private dependency.
That implementation dependency must be shipped in the same or an earlier
release before a stable artifact uses it, while the preview process symbols do
not become stable by association. The stdlib does not import `process`;
...
| `--color` | Requests automatic terminal colors (stderr TTY and `NO_COLOR` unset) and is removed from application argv. |
...
| process (preview, unreleased) | `base_process_owner_alive` | The predicate returns `0` for a directly parented owner/guardian relationship, `1` for a false relationship, and `2` for malformed PID inputs; it does not signal or mutate caller state. This surface is not in v2.0.0. |
