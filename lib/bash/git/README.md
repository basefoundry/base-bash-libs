...
branch=""
base_bash_libs_git_get_current_branch "$PWD" branch
base_bash_libs_std_log_info "Current branch: $branch"
```
...
- `base_bash_libs_git_update_repo` only attempts updates when the checked-out branch is the detected default branch, or an explicit expected branch passed by the caller.
- `base_bash_libs_git_update_repo` retries `git pull --ff-only` twice by default. Set
  `BASE_BASH_LIBS_GIT_PULL_MAX_ATTEMPTS` to a positive integer to change the retry count.
- `base_bash_libs_git_get_current_branch` uses `git -C` so it does not change the caller's
...
- `base_bash_libs_git_update_repo` changes into the target repository while it runs because
  its submodule update sequence depends on repository-relative execution.
- `base_bash_libs_git_update_repo` only treats an allowed dirty path as safe when every tracked
  change stays within that path. Rename records must have both source and
  destination inside the allowed path.
- `base_bash_libs_git_check_script_up_to_date` treats a missing file, unavailable Git executable,
...
- `base_bash_libs_git_check_script_up_to_date <script>` compares `HEAD` with the local remote-tracking upstream ref. It does not fetch by default, so the result reflects the freshness of local refs.
- `base_bash_libs_git_check_script_up_to_date --fetch <script>` runs `git fetch --quiet` first,
...
### `base_bash_libs_git_check_script_up_to_date` statuses
