...
#
# Core helpers:
#   base_bash_libs_std_run [opts] cmd ...
#                                # Safe command runner with dry-run, timeout, retry & failure handling.
#   base_bash_libs_std_exit_if_error rc msg...      # Log + exit when rc != 0 (preserves original status).
#   base_bash_libs_std_fatal_error msg...           # Convenience wrapper: exit with last status or 1.
#   base_bash_libs_std_register_cleanup_hook fn # Run a cleanup function from the shared EXIT trap.
#   base_bash_libs_std_register_cleanup_path p  # Remove owned files/directories from EXIT cleanup.
#   base_bash_libs_std_register_cleanup_path --unsafe p
#                                # Explicitly opt out of path-identity proof.
#   base_bash_libs_std_unregister_cleanup_path p
#                                # Drop files/directories from the shared EXIT cleanup list.
#   base_bash_libs_std_make_temp_file var [pfx] # Create a temp file and store its path in var.
#   base_bash_libs_std_make_temp_dir var [pfx]  # Create a temp directory and store its path in var.
#   base_bash_libs_std_command_path var cmd     # Resolve an external command path without exiting.
#   base_bash_libs_std_function_exists fn       # Predicate for defined Bash functions.
#   base_bash_libs_std_assert_variable_name name    # Validate Bash variable-name arguments.
#   base_bash_libs_std_assert_associative_array map # Validate caller-declared associative arrays.
#   base_bash_libs_require_version min_version
#                                # Exit clearly if the loaded library is too old.
#   base_bash_libs_std_add_to_path [-n] [-p] dir    # Append/prepend unique PATH entries.
#   base_bash_libs_std_set_log_level [LEVEL]        # Adjust terminal verbosity (FATAL..VERBOSE).
#   base_bash_libs_std_set_log_category_level -l category LEVEL
#                                # Gate a category independently of its sinks.
#   base_bash_libs_std_log_is_enabled [-l category] LEVEL
#                                # Test whether any configured sink accepts a level.
#   base_bash_libs_std_log_info/debug/... msgs      # Structured logging (color in interactive shells).
#   base_bash_libs_std_safe_touch file [...]        # touch wrapper that exits on failure (same for base_bash_libs_std_safe_truncate).
#   assert_* utilities           # Validation helpers (base_bash_libs_std_assert_not_null / base_bash_libs_std_assert_integer / ...).
#
# Patterns:
#   base_bash_libs_std_run some_cmd             # exits on failure; BASE_BASH_LIBS_DRY_RUN=true/1/yes/on prints instead.
#   base_bash_libs_std_run --timeout 30 some_cmd
#                                # bounds the command attempt to 30 seconds.
#   base_bash_libs_std_run --max-attempts 3 --retry-delay 2 some_cmd
#                                # retries transient failures.
#   some_cmd || base_bash_libs_std_fatal_error ...  # preserves failing exit code before terminating.
#   base_bash_libs_std_add_to_path -p "/opt/tools"  # inject directories without duplicates.
#
# Notes:
...
base_bash_libs_require_version() {
    ...
    if ! __base_bash_libs_std_is_dotted_numeric_version__ "$minimum_version" ||
        ...
        base_bash_libs_std_fatal_error "base_bash_libs_require_version expects dotted numeric versions."
    fi

    if ! __base_bash_libs_std_version_at_least__ "$BASE_BASH_LIBS_VERSION" "$minimum_version"; then
        base_bash_libs_std_fatal_error "base-bash-libs $minimum_version or newer is required; loaded version is $BASE_BASH_LIBS_VERSION."
    fi
...
#
# base_bash_libs_std_is_interactive - Checks if the current shell is interactive.
#
...
#
base_bash_libs_std_is_interactive() {
    [[ -t 0 ]]
}

#
# base_bash_libs_std_check_bash_version - Verifies the Bash version without prompting or installing anything.
#
            ...
            case "$arg" in
                ...
                --verbose-wrapper)
                    if ((configure_runtime)); then
                        ...
                        base_bash_libs_std_set_log_category_level -l base_bash_libs VERBOSE
                        export BASE_BASH_LIBS_LOG_DEBUG=1
                    fi
                    ;;
                --utc-wrapper)
...
#
# base_bash_libs_std_import - Sources one or more other library files.
#
...
#
# Usage:
#   base_bash_libs_std_import /path/to/absolute/lib.sh
#   base_bash_libs_std_import relative/path/to/lib2.sh
#
...
#
base_bash_libs_std_import() {
    local lib import_path
    for lib; do
        import_path="$lib"
        if [[ "$lib" != /* ]]; then
           [[ -n "${BASE_BASH_LIBS_SCRIPT_DIR-}" ]] || { printf '%s\n' "ERROR: base_bash_libs_init must run before relative imports" >&2; exit 1; }
           import_path="$BASE_BASH_LIBS_SCRIPT_DIR/$lib"
        fi
        if [[ -f "$import_path" ]]; then
            # shellcheck disable=SC1090
            source "$import_path"
            base_bash_libs_std_exit_if_error $? "Import of library '$lib' not successful."
        else
            base_bash_libs_std_exit_if_error 1 "Library '$lib' does not exist"
        fi
    done
    return 0
}
...
base_bash_libs_std_add_to_path() {
    ...
    while getopts np opt; do
        case "$opt" in
            ...
            *)  base_bash_libs_std_log_error -l base_bash_libs.std "base_bash_libs_std_add_to_path: invalid option '$opt'"
                return 1
                ;;
        esac
    done
...
#
# base_bash_libs_std_dedupe_path - Removes duplicate entries from the PATH variable.
#
base_bash_libs_std_dedupe_path() {
    local -A seen
...
#
# base_bash_libs_std_print_path - Prints each directory in the PATH on a new line.
#
...
#
# base_bash_libs_std_set_log_level - Sets the logging verbosity for a given logger.
#
# Usage:
#   base_bash_libs_std_set_log_level [level]
#   base_bash_libs_std_set_log_level -l [logger_name] [level]
#
# Arguments:
...
#
base_bash_libs_std_set_log_level() {
...
#
# base_bash_libs_std_set_log_category_level - Sets the gate for a hierarchical log category.
#
# Usage:
#   base_bash_libs_std_set_log_category_level -l [category] [level]
#
...
#
base_bash_libs_std_set_log_category_level() {
    ...
    if [[ "$#" -ne 3 || "${1-}" != "-l" || -z "${2-}" || -z "${3-}" ]]; then
        __base_bash_libs_std_log_source_location__ __base_bash_libs_std_set_category_source_location \
            "${BASH_SOURCE[1]:-${0:-unknown}}" "${BASH_LINENO[0]:-0}"
        printf '%(%Y-%m-%d:%H:%M:%S)T %-7s %s\n' -1 WARN \
            "$__base_bash_libs_std_set_category_source_location Usage: base_bash_libs_std_set_log_category_level -l <category> <level>" >&2
        return 1
    fi
...
#
# base_bash_libs_std_log_is_enabled - Return success when any configured sink accepts a level.
#
# Usage:
#   base_bash_libs_std_log_is_enabled [-l category] level
#
base_bash_libs_std_log_is_enabled() {
    ...
    if [[ "$#" -ne 1 || -z "${1-}" ]]; then
        __base_bash_libs_std_log_source_location__ __base_bash_libs_std_log_enabled_source_location \
            "${BASH_SOURCE[1]:-${0:-unknown}}" "${BASH_LINENO[0]:-0}"
        printf '%(%Y-%m-%d:%H:%M:%S)T %-7s %s\n' -1 WARN \
            "$__base_bash_libs_std_log_enabled_source_location Usage: base_bash_libs_std_log_is_enabled [-l <category>] <level>" >&2
        return 1
    fi
...
#
# Internal helper to be called by `base_bash_libs_std_log_info_file`, etc.
#
...
#
base_bash_libs_std_log_fatal()   { __base_bash_libs_std_print_log__ FATAL   "$@"; }
base_bash_libs_std_log_error()   { __base_bash_libs_std_print_log__ ERROR   "$@"; }
base_bash_libs_std_log_warn()    { __base_bash_libs_std_print_log__ WARN    "$@"; }
base_bash_libs_std_log_info()    { __base_bash_libs_std_print_log__ INFO    "$@"; }
base_bash_libs_std_log_debug()   { __base_bash_libs_std_print_log__ DEBUG   "$@"; }
# Deprecated compatibility helper; prefer base_bash_libs_std_log_debug.
base_bash_libs_std_log_verbose() { __base_bash_libs_std_print_log__ VERBOSE "$@"; }

#
# Public functions for logging the content of a file.
#
base_bash_libs_std_log_info_file()    { __base_bash_libs_std_print_log_file__ INFO    "$@"; }
base_bash_libs_std_log_debug_file()   { __base_bash_libs_std_print_log_file__ DEBUG   "$@"; }
# Deprecated compatibility helper; prefer base_bash_libs_std_log_debug_file.
base_bash_libs_std_log_verbose_file() { __base_bash_libs_std_print_log_file__ VERBOSE "$@"; }

#
# Public functions for logging function entry and exit points.
#
base_bash_libs_std_log_info_enter()    { __base_bash_libs_std_print_log__ INFO    "Entering function ${FUNCNAME[1]:-main}"; }
base_bash_libs_std_log_debug_enter()   { __base_bash_libs_std_print_log__ DEBUG   "Entering function ${FUNCNAME[1]:-main}"; }
# Deprecated compatibility helper; prefer base_bash_libs_std_log_debug_enter.
base_bash_libs_std_log_verbose_enter() { __base_bash_libs_std_print_log__ VERBOSE "Entering function ${FUNCNAME[1]:-main}"; }
base_bash_libs_std_log_info_leave()    { __base_bash_libs_std_print_log__ INFO    "Leaving function ${FUNCNAME[1]:-main}";  }
base_bash_libs_std_log_debug_leave()   { __base_bash_libs_std_print_log__ DEBUG   "Leaving function ${FUNCNAME[1]:-main}";  }
# Deprecated compatibility helper; prefer base_bash_libs_std_log_debug_leave.
base_bash_libs_std_log_verbose_leave() { __base_bash_libs_std_print_log__ VERBOSE "Leaving function ${FUNCNAME[1]:-main}";  }

#
# Simple print routines that do not prefix messages with timestamps or levels.
#
base_bash_libs_std_print_error()   { local __base_bash_libs_std_print_error_message; __base_bash_libs_std_print_error_message="$(__base_bash_libs_std_join_message__ "$@")"; { printf '%bERROR: %s%b\n' "$BASE_BASH_LIBS_STD_COLOR_RED" "$__base_bash_libs_std_print_error_message" "$BASE_BASH_LIBS_STD_COLOR_OFF"; } >&2; }
base_bash_libs_std_print_warn()    { local __base_bash_libs_std_print_warn_message; __base_bash_libs_std_print_warn_message="$(__base_bash_libs_std_join_message__ "$@")"; { printf '%bWARN: %s%b\n' "$BASE_BASH_LIBS_STD_COLOR_YELLOW" "$__base_bash_libs_std_print_warn_message" "$BASE_BASH_LIBS_STD_COLOR_OFF"; } >&2; }
base_bash_libs_std_print_info()    { local __base_bash_libs_std_print_info_message; __base_bash_libs_std_print_info_message="$(__base_bash_libs_std_join_message__ "$@")"; { printf '%b%s%b\n' "$BASE_BASH_LIBS_STD_COLOR_GREEN" "$__base_bash_libs_std_print_info_message" "$BASE_BASH_LIBS_STD_COLOR_OFF"; } >&2; }
base_bash_libs_std_print_success() { local __base_bash_libs_std_print_success_message; __base_bash_libs_std_print_success_message="$(__base_bash_libs_std_join_message__ "$@")"; { printf '%bSUCCESS: %s%b\n' "$BASE_BASH_LIBS_STD_COLOR_GREEN" "$__base_bash_libs_std_print_success_message" "$BASE_BASH_LIBS_STD_COLOR_OFF"; } >&2; }
base_bash_libs_std_print_bold()    { local __base_bash_libs_std_print_bold_message; __base_bash_libs_std_print_bold_message="$(__base_bash_libs_std_join_message__ "$@")"; printf '%b%s%b\n' "$BASE_BASH_LIBS_STD_COLOR_BOLD" "$__base_bash_libs_std_print_bold_message" "$BASE_BASH_LIBS_STD_COLOR_OFF"; }
base_bash_libs_std_print_message() { printf '%s\n' "$@"; }

#
# base_bash_libs_std_print_tty - Prints a message only if the output is going to a terminal.
#
base_bash_libs_std_print_tty() {
    if [[ -t 1 ]]; then
        printf '%s\n' "$(__base_bash_libs_std_join_message__ "$@")"
    fi
}
...
#
# base_bash_libs_std_dump_trace - Prints a stack trace of the Bash function calls.
#
...
#
# base_bash_libs_std_exit_if_error - Exits the script if the provided exit code is non-zero.
#
...
#   base_bash_libs_std_exit_if_error $? "A descriptive error message."
#
# Arguments:
#   $1: The exit code to check (typically $?).
#   $@: The error message to log if the exit code is non-zero.
#
base_bash_libs_std_exit_if_error() {
    (($#)) || return
    ...
    if ! [[ $__base_bash_libs_std_exit_status =~ $__base_bash_libs_std_exit_number_re ]]; then
        base_bash_libs_std_log_error -l base_bash_libs.std \
        ...
        base_bash_libs_std_log_error -l base_bash_libs.std "'$__base_bash_libs_std_exit_status' is not a valid decimal exit code. Treating it as 1."
        __base_bash_libs_std_exit_status=1
    else
        __base_bash_libs_std_exit_status="$__base_bash_libs_std_exit_normalized_status"
    fi
    ((__base_bash_libs_std_exit_status)) && {
        base_bash_libs_std_log_fatal -l base_bash_libs.std "$__base_bash_libs_std_exit_message"
        base_bash_libs_std_dump_trace
...
#
# base_bash_libs_std_fatal_error - A convenience wrapper around base_bash_libs_std_exit_if_error.
#
...
#
# Usage:
#   [[ -f "$my_file" ]] || base_bash_libs_std_fatal_error "Required file '$my_file' not found."
#
base_bash_libs_std_fatal_error() {
    ...
    base_bash_libs_std_exit_if_error "$__base_bash_libs_std_fatal_status" "$@"
}

#################################################### COMMAND EXECUTION #################################################

#
# base_bash_libs_std_is_dry_run - Returns true when dry-run mode is enabled.
#
# Dry-run mode may be enabled through either BASE_BASH_LIBS_DRY_RUN or BASE_BASH_LIBS_DRY_RUN. Both names
...
base_bash_libs_std_is_dry_run() {
    ...
    for __base_bash_libs_std_dry_run_value in "${BASE_BASH_LIBS_DRY_RUN-}" "${BASE_BASH_LIBS_DRY_RUN-}"; do
...
#
# base_bash_libs_std_run - Safely executes a simple command with its arguments.
#
...
#
# Usage:
#   base_bash_libs_std_run [options] command [arg1] [arg2] ...
#   base_bash_libs_std_run --sensitive [--safe-display label] [options] -- command [arg1] ...
#
# Options:
...
#   base_bash_libs_std_run ls -l /tmp
#
#   # Run a command with spaces in an argument.
#   base_bash_libs_std_run touch "a file with spaces.txt"
#
#   # Run a command but don't exit the script on failure.
#   if ! base_bash_libs_std_run --no-exit grep "not_found" /etc/hosts; then
...
#   base_bash_libs_std_run rm -rf /some/important/path
#
#   # Protect credentials in framework-generated diagnostics.
#   base_bash_libs_std_run --sensitive --safe-display "upload release asset" -- \
...
__base_bash_libs_std_run_impl__() {
        ...
        case "${1-}" in
            ...
            --timeout)
                ...
                if (($# == 0)) || ! __base_bash_libs_std_is_positive_integer__ "${1-}"; then
                    base_bash_libs_std_log_error -l base_bash_libs.std "$helper_name: timeout seconds must be a positive integer."
                    return 1
                fi
            ...
            --max-attempts | --retry-attempts)
                shift
                if (($# == 0)) || ! __base_bash_libs_std_is_positive_integer__ "${1-}"; then
                    base_bash_libs_std_log_error -l base_bash_libs.std "$helper_name: max attempts must be a positive integer."
                    return 1
                fi
            ...
            --retry-delay)
                ...
                if (($# == 0)) || ! __base_bash_libs_std_is_non_negative_integer__ "${1-}"; then
                    base_bash_libs_std_log_error -l base_bash_libs.std "$helper_name: retry delay seconds must be a non-negative integer."
                    return 1
                fi
    ...
    if ((safe_display_set && ! sensitive)); then
        base_bash_libs_std_log_error -l base_bash_libs.std "$helper_name: --safe-display is valid only with --sensitive."
        return 1
    fi
    ...
    if [[ $# -eq 0 ]]; then
        base_bash_libs_std_log_error -l base_bash_libs.std "$helper_name: No command provided."
        return 1
    fi
    ...
    __base_bash_libs_std_render_command_display__ __base_bash_libs_std_run_immutable_command_display "$sensitive" "$safe_display" \
        ...
        base_bash_libs_std_log_error -l base_bash_libs.std "$helper_name: could not render a safe command diagnostic."
        return 1
    }
    ...
    # --- Dry-Run Handling ---
    if base_bash_libs_std_is_dry_run; then
    ...
    while ((__base_bash_libs_std_run_attempt_number <= __base_bash_libs_std_run_policy_max_attempts)); do
        ...
        if ((__base_bash_libs_std_run_attempt_number < __base_bash_libs_std_run_policy_max_attempts)); then
            if ((! __base_bash_libs_std_run_policy_quiet)); then
                ...
                base_bash_libs_std_log_warn -l base_bash_libs.std \
    ...
    if ((__base_bash_libs_std_run_exit_code)); then
        ...
        if ((__base_bash_libs_std_run_policy_exit_on_failure)); then
            base_bash_libs_std_exit_if_error "$__base_bash_libs_std_run_exit_code" "$__base_bash_libs_std_run_message"
        else
            if ((! __base_bash_libs_std_run_policy_quiet)); then
                base_bash_libs_std_log_warn -l base_bash_libs.std "$__base_bash_libs_std_run_message (continuing)."
            fi
...
base_bash_libs_std_run() {
    __base_bash_libs_std_run_impl__ base_bash_libs_std_run "$@"
}
...
__base_bash_libs_std_timeout_backend_detect__() {
    ...
    for __base_bash_libs_std_timeout_backend_name in timeout gtimeout; do
        ...
        if base_bash_libs_std_command_path timeout_backend_candidate \
...
#
# base_bash_libs_std_safe_mkdir: Attempt to create directories and exit on failure.
#             Creates as many directories as possible.
#
# Usage: base_bash_libs_std_safe_mkdir [-p] dir1 dir2 ...
#
base_bash_libs_std_safe_mkdir() {
    ...
    while getopts ":p" opt; do
        case "$opt" in
            p) mkdir_args=(-p) ;;
            \?)
                base_bash_libs_std_log_error -l base_bash_libs.std "base_bash_libs_std_safe_mkdir: invalid option '-$OPTARG'"
                return 1
                ;;
        esac
    done
    ...
    if (($# == 0)); then
        base_bash_libs_std_log_warn -l base_bash_libs.std "base_bash_libs_std_safe_mkdir: No directories provided to create."
        return 0
    fi
    ...
    [[ -z "${failed_dirs[0]+set}" ]] || base_bash_libs_std_exit_if_error 1 "Failed to create directories: ${failed_dirs[*]}"
    return 0
}

#
# base_bash_libs_std_safe_touch - Creates or updates the timestamp of one or more files.
#
...
#
# Usage:
#   base_bash_libs_std_safe_touch "/tmp/file1.log" "/var/run/app.pid"
#
# Arguments:
#   $@: One or more file paths to touch.
#
base_bash_libs_std_safe_touch() {
    local failed_files=()
    local file touch_path

    if (($# == 0)); then
        base_bash_libs_std_log_warn -l base_bash_libs.std "base_bash_libs_std_safe_touch: No files provided to touch."
        return 0
    fi
    ...
    if [[ -n "${failed_files[0]+set}" ]]; then
        base_bash_libs_std_fatal_error "Failed to touch the following files: ${failed_files[*]}"
    fi
...
#
# base_bash_libs_std_safe_truncate - Truncates one or more files to zero bytes.
#
# This function iterates through all provided file paths. It attempts to
...
#
# Usage:
#   base_bash_libs_std_safe_truncate "/var/log/app.log" "/tmp/data.tmp"
#
# Arguments:
#   $@: One or more file paths to truncate.
#
base_bash_libs_std_safe_truncate() {
    local failed_files=()
    local file

    if (($# == 0)); then
        base_bash_libs_std_log_warn -l base_bash_libs.std "base_bash_libs_std_safe_truncate: No files provided to truncate."
        return 0
    fi
    ...
    if [[ -n "${failed_files[0]+set}" ]]; then
        base_bash_libs_std_fatal_error "Failed to truncate the following files: ${failed_files[*]}"
    fi
...
__base_bash_libs_std_cleanup_delete_path__() {
    ...
    if [[ "$fingerprint" != UNSAFE ]] &&
        ...
        base_bash_libs_std_log_warn -l base_bash_libs.std "Cleanup path '$cleanup_path' changed identity; refusing to remove it."
        return 1
    fi
    if [[ "$fingerprint" == UNSAFE ]]; then
        base_bash_libs_std_log_warn -l base_bash_libs.std "Removing explicitly unsafe cleanup path '$cleanup_path'."
    fi
    if ! rm -rf -- "$cleanup_path"; then
        base_bash_libs_std_log_warn -l base_bash_libs.std "Cleanup path '$cleanup_path' could not be removed."
        return 1
    fi
    return 0
}
...
__base_bash_libs_std_run_cleanup_hooks__() {
    ...
    for ((index=${#__base_bash_libs_std_cleanup_entries[@]} - 1; index >= 0; index--)); do
        ...
        case "$entry_type" in
            hook)
                if ! "$entry_value"; then
                    base_bash_libs_std_log_warn -l base_bash_libs.std "Cleanup hook '$entry_value' failed."
                fi
                ;;
            path)
...
#
# base_bash_libs_std_register_cleanup_hook - Registers a function to run from the shared EXIT trap.
#
...
#
base_bash_libs_std_register_cleanup_hook() {
    local hook="${1-}" existing_hook

    if (($# != 1)); then
        base_bash_libs_std_log_error -l base_bash_libs.std "base_bash_libs_std_register_cleanup_hook: expected exactly one function name."
        return 1
    fi
    if ! __base_bash_libs_std_is_valid_variable_name__ "$hook" || ! declare -F "$hook" >/dev/null; then
        base_bash_libs_std_log_error -l base_bash_libs.std "base_bash_libs_std_register_cleanup_hook: '$hook' is not a defined cleanup function."
        return 1
    fi
...
#
# base_bash_libs_std_unregister_cleanup_hook - Removes a function from the shared EXIT cleanup hook list.
#
# Usage:
#   base_bash_libs_std_unregister_cleanup_hook cleanup_workspace
#
base_bash_libs_std_unregister_cleanup_hook() {
    local hook="${1-}" existing_hook entry entry_type entry_value
    local -a remaining_hooks=() remaining_entries=()

    if (($# != 1)); then
        base_bash_libs_std_log_error -l base_bash_libs.std "base_bash_libs_std_unregister_cleanup_hook: expected exactly one function name."
        return 1
    fi
...
#
# base_bash_libs_std_register_cleanup_path - Registers files or directories for removal at shell exit.
#
...
#   base_bash_libs_std_register_cleanup_path --unsafe "$legacy_path"
#
base_bash_libs_std_register_cleanup_path() {
    ...
    if (($# == 0)); then
        base_bash_libs_std_log_warn -l base_bash_libs.std "base_bash_libs_std_register_cleanup_path: No paths provided."
        return 0
    fi
    ...
    for path; do
        if ! __base_bash_libs_std_is_safe_cleanup_path__ "$path"; then
            base_bash_libs_std_log_error -l base_bash_libs.std "base_bash_libs_std_register_cleanup_path: refusing to register unsafe cleanup path '$path'."
            status=1
            continue
        fi
        if __base_bash_libs_std_is_broad_cleanup_path__ "$path"; then
            base_bash_libs_std_log_error -l base_bash_libs.std "base_bash_libs_std_register_cleanup_path: refusing to register broad or protected path '$path'."
            status=1
            continue
        fi
        if (( ! unsafe )) && [[ -L "$path" ]]; then
            base_bash_libs_std_log_error -l base_bash_libs.std "base_bash_libs_std_register_cleanup_path: refusing to register symlink '$path' without --unsafe."
            status=1
            continue
        fi
        if (( ! unsafe )) && ! [[ -e "$path" ]]; then
            base_bash_libs_std_log_error -l base_bash_libs.std "base_bash_libs_std_register_cleanup_path: path '$path' does not exist for ownership proof."
            status=1
            continue
        fi
        ...
        if (( ! already_registered )); then
            if (( unsafe )); then
                ...
                base_bash_libs_std_log_error -l base_bash_libs.std "base_bash_libs_std_register_cleanup_path: unable to snapshot ownership for '$path'."
                status=1
                continue
            fi
...
#
# base_bash_libs_std_unregister_cleanup_path - Removes files or directories from the shared EXIT cleanup path list.
#
...
#
base_bash_libs_std_unregister_cleanup_path() {
    ...
    if (($# == 0)); then
        base_bash_libs_std_log_warn -l base_bash_libs.std "base_bash_libs_std_unregister_cleanup_path: No paths provided."
        return 0
    fi

    for path; do
        if ! __base_bash_libs_std_is_safe_cleanup_path__ "$path"; then
            base_bash_libs_std_log_error -l base_bash_libs.std "base_bash_libs_std_unregister_cleanup_path: refusing to unregister unsafe cleanup path '$path'."
            status=1
            continue
        fi
...
__base_bash_libs_std_make_temp_path__() {
    ...
    if (($# < 1 || $# > 2)); then
        base_bash_libs_std_log_error -l base_bash_libs.std "$__base_bash_libs_std_temp_helper_name: usage: $__base_bash_libs_std_temp_helper_name [--keep] <result_variable_name> [prefix]"
        return 1
    fi
    ...
    if ! __base_bash_libs_std_is_valid_variable_name__ "$__base_bash_libs_std_temp_result_name"; then
        base_bash_libs_std_log_error -l base_bash_libs.std "$__base_bash_libs_std_temp_helper_name: result variable name must be a valid Bash variable name."
        return 1
    fi
    ...
    if [[ -z "$__base_bash_libs_std_temp_prefix" || "$__base_bash_libs_std_temp_prefix" == */* ]]; then
        base_bash_libs_std_log_error -l base_bash_libs.std "$__base_bash_libs_std_temp_helper_name: prefix must be a non-empty filename prefix without '/'."
        return 1
    fi
    ...
    if [[ "$__base_bash_libs_std_temp_root" != /* ]]; then
        if ! __base_bash_libs_std_temp_root="$(cd -- "$__base_bash_libs_std_temp_root" 2>/dev/null && pwd -P)"; then
            base_bash_libs_std_log_error -l base_bash_libs.std "$__base_bash_libs_std_temp_helper_name: TMPDIR is not a directory: ${TMPDIR:-/tmp}"
            return 1
        fi
    fi
    ...
    if [[ "$__base_bash_libs_std_temp_path_kind" == "dir" ]]; then
        __base_bash_libs_std_temp_path="$(mktemp -d "$__base_bash_libs_std_temp_template" 2>/dev/null)" || {
            base_bash_libs_std_log_error -l base_bash_libs.std "$__base_bash_libs_std_temp_helper_name: failed to create temporary directory."
            return 1
        }
    else
        __base_bash_libs_std_temp_path="$(mktemp "$__base_bash_libs_std_temp_template" 2>/dev/null)" || {
            base_bash_libs_std_log_error -l base_bash_libs.std "$__base_bash_libs_std_temp_helper_name: failed to create temporary file."
            return 1
        }
    fi

    if ((! __base_bash_libs_std_temp_keep)); then
        if ! base_bash_libs_std_register_cleanup_path "$__base_bash_libs_std_temp_path"; then
...
#
# base_bash_libs_std_make_temp_file - Creates a temporary file and stores its path in a named variable.
#
# The created file is registered for exit cleanup unless `--keep` is provided.
#
# Usage:
#   base_bash_libs_std_make_temp_file [--keep] <result_variable_name> [prefix]
#
base_bash_libs_std_make_temp_file() {
    __base_bash_libs_std_preflight_temp_result_name__ base_bash_libs_std_make_temp_file "$@" || return 1
    __base_bash_libs_std_make_temp_path__ base_bash_libs_std_make_temp_file file "$@"
}
...
#
# base_bash_libs_std_make_temp_dir - Creates a temporary directory and stores its path in a named variable.
#
# The created directory is registered for exit cleanup unless `--keep` is provided.
#
# Usage:
#   base_bash_libs_std_make_temp_dir [--keep] <result_variable_name> [prefix]
#
base_bash_libs_std_make_temp_dir() {
    __base_bash_libs_std_preflight_temp_result_name__ base_bash_libs_std_make_temp_dir "$@" || return 1
    __base_bash_libs_std_make_temp_path__ base_bash_libs_std_make_temp_dir dir "$@"
}
...
#
# base_bash_libs_std_assert_variable_name - Verifies that one or more arguments are valid Bash variable names.
#
# This validates the names themselves. It does not require the named variables to
# exist or have non-empty values.
#
# Usage:
#   base_bash_libs_std_assert_variable_name result_name array_name
#
base_bash_libs_std_assert_variable_name() {
    local __base_bash_libs_std_assert_variable_name

    if (($# == 0)); then
        base_bash_libs_std_fatal_error "base_bash_libs_std_assert_variable_name: No variable names provided for validation."
    fi
    ...
    for __base_bash_libs_std_assert_variable_name in "$@"; do
        if ! __base_bash_libs_std_is_valid_variable_name__ "$__base_bash_libs_std_assert_variable_name"; then
            base_bash_libs_std_fatal_error "base_bash_libs_std_assert_variable_name expects valid Bash variable names; one or more arguments are invalid."
        fi
    done
...
#
# base_bash_libs_std_assert_indexed_array - Verifies that one or more variables are declared indexed arrays.
#
...
base_bash_libs_std_assert_indexed_array() {
    if (($# == 0)); then
        base_bash_libs_std_fatal_error "base_bash_libs_std_assert_indexed_array: No variable names provided for validation."
    fi
    __base_bash_libs_std_assert_public_variable_names__ base_bash_libs_std_assert_indexed_array "$@" || return 1
    local __base_bash_libs_std_assert_indexed_name

    for __base_bash_libs_std_assert_indexed_name in "$@"; do
        base_bash_libs_std_assert_variable_name "$__base_bash_libs_std_assert_indexed_name"
        if ! __base_bash_libs_std_declares_array_kind__ "$__base_bash_libs_std_assert_indexed_name" "a"; then
            base_bash_libs_std_fatal_error "Variable '$__base_bash_libs_std_assert_indexed_name' must be an indexed array declared by the caller."
        fi
    done
...
#
# base_bash_libs_std_assert_associative_array - Verifies that one or more variables are declared associative arrays.
#
...
#
base_bash_libs_std_assert_associative_array() {
    if (($# == 0)); then
        base_bash_libs_std_fatal_error "base_bash_libs_std_assert_associative_array: No variable names provided for validation."
    fi
    __base_bash_libs_std_assert_public_variable_names__ base_bash_libs_std_assert_associative_array "$@" || return 1
    local __base_bash_libs_std_assert_associative_name

    for __base_bash_libs_std_assert_associative_name in "$@"; do
        base_bash_libs_std_assert_variable_name "$__base_bash_libs_std_assert_associative_name"
        if ! __base_bash_libs_std_declares_array_kind__ "$__base_bash_libs_std_assert_associative_name" "A"; then
            base_bash_libs_std_fatal_error "Variable '$__base_bash_libs_std_assert_associative_name' must be an associative array declared by the caller."
        fi
    done
...
#
# base_bash_libs_std_command_path - Resolves an external command path without exiting the caller.
#
# Usage:
...
base_bash_libs_std_command_path() {
    if (($# != 2)); then
        base_bash_libs_std_log_error -l base_bash_libs.std "base_bash_libs_std_command_path: usage: base_bash_libs_std_command_path <result_variable_name> <command_name>"
        return 1
    fi
    __base_bash_libs_std_assert_public_variable_names__ base_bash_libs_std_command_path "${1-}" || return 1
    ...
    if ! __base_bash_libs_std_is_valid_variable_name__ "$__base_bash_libs_std_command_result_name"; then
        base_bash_libs_std_log_error -l base_bash_libs.std "base_bash_libs_std_command_path: result variable name must be a valid Bash variable name."
        return 1
    fi
    __base_bash_libs_std_assert_writable_output__ base_bash_libs_std_command_path "$__base_bash_libs_std_command_result_name" || return 1
...
#
# base_bash_libs_std_function_exists - Checks whether a Bash function is currently defined.
#
base_bash_libs_std_function_exists() {
...
#
# base_bash_libs_std_assert_function_exists - Verifies that one or more Bash functions are defined.
#
# Usage:
#   base_bash_libs_std_assert_function_exists main cleanup_workspace
#
base_bash_libs_std_assert_function_exists() {
    local missing_functions=() function_name

    if (($# == 0)); then
        base_bash_libs_std_fatal_error "base_bash_libs_std_assert_function_exists: No function names provided for validation."
    fi

    for function_name in "$@"; do
        if ! __base_bash_libs_std_is_valid_variable_name__ "$function_name"; then
            base_bash_libs_std_fatal_error "base_bash_libs_std_assert_function_exists expects function names; one or more arguments are not valid Bash function names."
        fi
        if ! base_bash_libs_std_function_exists "$function_name"; then
            missing_functions+=("$function_name")
        fi
    done

    if [[ -n "${missing_functions[0]+set}" ]]; then
        base_bash_libs_std_fatal_error "Required functions are not defined: ${missing_functions[*]}"
    fi
...
#
# base_bash_libs_std_assert_not_null - Checks that one or more variables are not empty.
#
...
#   USER="admin"
#   TOKEN=""
#   base_bash_libs_std_assert_not_null USER       # This will succeed.
#   base_bash_libs_std_assert_not_null USER TOKEN # This will fail, listing TOKEN as empty.
#   base_bash_libs_std_assert_not_null "$TOKEN"   # Wrong: pass variable names, not values.
#
# Arguments:
#   $@: One or more variable names to check.
#
base_bash_libs_std_assert_not_null() {
    if (($# == 0)); then
        base_bash_libs_std_fatal_error "base_bash_libs_std_assert_not_null: No variable names provided for validation."
    fi
    __base_bash_libs_std_assert_public_variable_names__ base_bash_libs_std_assert_not_null "$@" || return 1
    ...
    for __base_bash_libs_std_assert_not_null_name in "$@"; do
        if ! __base_bash_libs_std_is_valid_variable_name__ "$__base_bash_libs_std_assert_not_null_name"; then
            base_bash_libs_std_fatal_error "base_bash_libs_std_assert_not_null expects variable names, not values; one or more arguments are not valid Bash variable names."
        fi
    ...
    if [[ -n "${__base_bash_libs_std_assert_not_null_unset_names[0]+set}" ]]; then
        base_bash_libs_std_fatal_error "These required variables are not set or are empty: ${__base_bash_libs_std_assert_not_null_unset_names[*]}"
    fi
...
#
# base_bash_libs_std_assert_integer - Checks if the values of one or more variables are valid integers.
#
__base_bash_libs_std_assert_integer_names__() {
    ...
    for __base_bash_libs_std_assert_integer_name in "$@"; do
        if ! __base_bash_libs_std_is_valid_variable_name__ "$__base_bash_libs_std_assert_integer_name"; then
            base_bash_libs_std_fatal_error "base_bash_libs_std_assert_integer expects variable names, not values; one or more arguments are not valid Bash variable names."
        fi
        ...
        ! [[ "$__base_bash_libs_std_assert_integer_value" =~ $__base_bash_libs_std_assert_integer_re ]] &&
            base_bash_libs_std_fatal_error "Variable '$__base_bash_libs_std_assert_integer_name' with value '$__base_bash_libs_std_assert_integer_value' is not a valid integer."
    done
    return 0
}

base_bash_libs_std_assert_integer() {
    (($# == 0)) && base_bash_libs_std_fatal_error "base_bash_libs_std_assert_integer: No variable names provided."
    __base_bash_libs_std_assert_public_variable_names__ base_bash_libs_std_assert_integer "$@" || return 1
    __base_bash_libs_std_assert_integer_names__ "$@"
}

#
# base_bash_libs_std_assert_integer_range - Checks if a variable's value is an integer within a specified range.
#
# Arguments:
...
#   $3: The maximum value.
#
base_bash_libs_std_assert_integer_range() {
    (($# != 3)) && base_bash_libs_std_fatal_error "base_bash_libs_std_assert_integer_range: Expected 3 arguments, got $#."
    __base_bash_libs_std_assert_public_variable_names__ base_bash_libs_std_assert_integer_range "${1-}" || return 1
    ...
    if ! __base_bash_libs_std_is_valid_variable_name__ "$__base_bash_libs_std_range_name"; then
        base_bash_libs_std_fatal_error "base_bash_libs_std_assert_integer_range expects a variable name as its first argument."
    fi
    if ! [[ "$__base_bash_libs_std_range_min" =~ ^[-+]?[0-9]+$ ]]; then
        base_bash_libs_std_fatal_error "base_bash_libs_std_assert_integer_range minimum bound '$__base_bash_libs_std_range_min' is not a valid integer."
    fi
    if ! [[ "$__base_bash_libs_std_range_max" =~ ^[-+]?[0-9]+$ ]]; then
        base_bash_libs_std_fatal_error "base_bash_libs_std_assert_integer_range maximum bound '$__base_bash_libs_std_range_max' is not a valid integer."
    fi
    ...
    ((__base_bash_libs_std_range_min_number > __base_bash_libs_std_range_max_number)) &&
        base_bash_libs_std_fatal_error "base_bash_libs_std_assert_integer_range minimum '$__base_bash_libs_std_range_min' cannot exceed maximum '$__base_bash_libs_std_range_max'."
    ((__base_bash_libs_std_range_value_number < __base_bash_libs_std_range_min_number || __base_bash_libs_std_range_value_number > __base_bash_libs_std_range_max_number)) &&
        base_bash_libs_std_fatal_error "Variable '$__base_bash_libs_std_range_name' ($__base_bash_libs_std_range_value) is out of range [$__base_bash_libs_std_range_min, $__base_bash_libs_std_range_max]."
    return 0
}

#
# base_bash_libs_std_assert_arg_count - Checks that the number of arguments falls within a given range.
#
# Usage:
#   base_bash_libs_std_assert_arg_count $# 2      # Fails if arg count is not exactly 2
#   base_bash_libs_std_assert_arg_count $# 1 3    # Fails if arg count is not between 1 and 3 (inclusive)
#
# Arguments:
...
#
base_bash_libs_std_assert_arg_count() {
    ...
    if ((__base_bash_libs_std_arg_count_arity < 2 || __base_bash_libs_std_arg_count_arity > 3)); then
        base_bash_libs_std_fatal_error "base_bash_libs_std_assert_arg_count: Incorrect usage. Expected 2 or 3 arguments, but got $__base_bash_libs_std_arg_count_arity."
    fi

    # Create temporary named variables for base_bash_libs_std_assert_integer to check
    ...
    if [[ -n "$__base_bash_libs_std_arg_count_second" ]]; then
        ...
        ((__base_bash_libs_std_arg_count_first_number > __base_bash_libs_std_arg_count_second_number)) &&
            base_bash_libs_std_fatal_error "base_bash_libs_std_assert_arg_count minimum '$__base_bash_libs_std_arg_count_first' cannot exceed maximum '$__base_bash_libs_std_arg_count_second'."
    fi
    ...
    if [[ -z "$__base_bash_libs_std_arg_count_second" ]]; then
        ...
        if ((__base_bash_libs_std_arg_count_actual_number != __base_bash_libs_std_arg_count_first_number)); then
            base_bash_libs_std_fatal_error "Argument count mismatch: expected $__base_bash_libs_std_arg_count_first but got $__base_bash_libs_std_arg_count_actual arguments"
        fi
    else
        # Range match case
        if ((__base_bash_libs_std_arg_count_actual_number < __base_bash_libs_std_arg_count_first_number ||
            ...
            base_bash_libs_std_fatal_error "Argument count mismatch: expected between $__base_bash_libs_std_arg_count_first and $__base_bash_libs_std_arg_count_second arguments, but got $__base_bash_libs_std_arg_count_actual"
        fi
    fi
    return 0
}
...
#
# base_bash_libs_std_assert_command_exists - Checks that one or more commands are available in the system's PATH.
#
...
#
# Usage:
#   base_bash_libs_std_assert_command_exists git curl jq
#
# Arguments:
...
base_bash_libs_std_assert_command_exists() {
    ...
    if (($# == 0)); then
        base_bash_libs_std_log_warn -l base_bash_libs.std "base_bash_libs_std_assert_command_exists: No commands provided to check."
        return 0
    fi
    ...
    if [[ -n "${missing_commands[0]+set}" ]]; then
        base_bash_libs_std_fatal_error "These required commands were not found in your PATH: ${missing_commands[*]}"
    fi
...
#
# base_bash_libs_std_assert_file_exists - Checks that one or more paths exist and are regular files.
#
...
#
# Usage:
#   base_bash_libs_std_assert_file_exists "/etc/hosts" "./my_script.sh"
#
# Arguments:
...
base_bash_libs_std_assert_file_exists() {
    ...
    if (($# == 0)); then
        base_bash_libs_std_log_warn -l base_bash_libs.std "base_bash_libs_std_assert_file_exists: No files provided to check."
        return 0
    fi
    ...
    if [[ -n "${missing_files[0]+set}" ]]; then
        base_bash_libs_std_fatal_error "These required files do not exist or are not regular files: ${missing_files[*]}"
    fi
...
#
# base_bash_libs_std_assert_executable - Checks that one or more paths exist and are executable files.
#
# This function iterates through all provided paths. If any path does not
...
# `base_bash_libs_std_assert_command_exists` when checking whether a command is discoverable
# through PATH.
#
# Usage:
#   base_bash_libs_std_assert_executable "./bin/tool" "/opt/vendor/bin/tool"
#
# Arguments:
#   $@: One or more executable file paths to check.
#
base_bash_libs_std_assert_executable() {
    local missing_executables=()
    local executable

    if (($# == 0)); then
        base_bash_libs_std_log_warn -l base_bash_libs.std "base_bash_libs_std_assert_executable: No executable paths provided to check."
        return 0
    fi
    ...
    if [[ -n "${missing_executables[0]+set}" ]]; then
        base_bash_libs_std_fatal_error "These required executable paths do not exist, are not regular files, or are not executable: ${missing_executables[*]}"
    fi
...
#
# base_bash_libs_std_assert_dir_exists - Checks that one or more paths exist and are directories.
#
# This function iterates through all provided paths. If any path does not
...
#
# Usage:
#   base_bash_libs_std_assert_dir_exists "/tmp" "/var/log"
#
# Arguments:
#   $@: One or more directory paths to check.
#
base_bash_libs_std_assert_dir_exists() {
    local missing_dirs=()
    local dir

    if (($# == 0)); then
        base_bash_libs_std_log_warn -l base_bash_libs.std "base_bash_libs_std_assert_dir_exists: No directories provided to check."
        return 0
    fi
    ...
    if [[ -n "${missing_dirs[0]+set}" ]]; then
        base_bash_libs_std_fatal_error "These required directories do not exist: ${missing_dirs[*]}"
    fi
...
#
# base_bash_libs_std_safe_cd - A safe version of the 'cd' command that exits on failure.
#
base_bash_libs_std_safe_cd() {
    local dir="${1-}"
    [[ "$dir" ]] || base_bash_libs_std_fatal_error "No arguments or an empty string passed to base_bash_libs_std_safe_cd"
    cd -- "$dir" || base_bash_libs_std_fatal_error "Can't cd to '$dir'"
}

#
# base_bash_libs_std_safe_unalias - Safely unaliases a command, without erroring if it doesn't exist.
#
base_bash_libs_std_safe_unalias() {
...
#
# base_bash_libs_std_get_my_source_dir - Returns the absolute path to the directory of the calling script through the passed variable name.
#
# Usage:
#   base_bash_libs_std_get_my_source_dir var_name
#
base_bash_libs_std_get_my_source_dir() {
    [[ -n "${1-}" ]] || base_bash_libs_std_fatal_error "base_bash_libs_std_get_my_source_dir: No result variable name provided."
    __base_bash_libs_std_assert_public_variable_names__ base_bash_libs_std_get_my_source_dir "${1-}" || return 1
    ...
    if ! __base_bash_libs_std_is_valid_variable_name__ "$__base_bash_libs_std_source_result_name"; then
        base_bash_libs_std_fatal_error "base_bash_libs_std_get_my_source_dir: result variable name must be a valid Bash variable name."
    fi
    __base_bash_libs_std_assert_writable_output__ base_bash_libs_std_get_my_source_dir "$__base_bash_libs_std_source_result_name" || return 1
    ...
    if [[ -n "$__base_bash_libs_std_source_path" ]]; then
        __base_bash_libs_std_source_dir="$(cd "$(dirname -- "$__base_bash_libs_std_source_path")" >/dev/null 2>&1 && pwd -P)" ||
            base_bash_libs_std_fatal_error "base_bash_libs_std_get_my_source_dir: Unable to resolve source directory."
    else
        __base_bash_libs_std_source_dir="$(pwd -P)" || base_bash_libs_std_fatal_error "base_bash_libs_std_get_my_source_dir: Unable to resolve source directory."
    fi
    printf -v "$__base_bash_libs_std_source_result_name" '%s' "$__base_bash_libs_std_source_dir"
}

#
# base_bash_libs_std_ask_yes_no - Get user's confirmation
#
...
# Usage:
#
#   if base_bash_libs_std_ask_yes_no "Do you want to continue?"; then
#       echo "User chose to continue."
#   else
#       echo "User chose not to continue."
#   fi
#
base_bash_libs_std_ask_yes_no() {
    if (("$#" != 1)); then
        base_bash_libs_std_log_error -l base_bash_libs.std "base_bash_libs_std_ask_yes_no: invalid arguments"
        base_bash_libs_std_log_info -l base_bash_libs.std "Usage: base_bash_libs_std_ask_yes_no <prompt_message>"
        return 1
    fi

    local message=$1 user_input tty_fd
    if ! exec {tty_fd}</dev/tty 2>/dev/null; then
        base_bash_libs_std_log_error -l base_bash_libs.std "base_bash_libs_std_ask_yes_no: /dev/tty is not available"
        return 1
    fi
...
#
# base_bash_libs_std_wait_for_enter - Pauses the script and waits for the user to press the Enter key.
#
# Arguments:
#   $1: (Optional) The prompt to display. Defaults to "Press Enter to continue".
#
base_bash_libs_std_wait_for_enter() {
    if (("$#" > 1)); then
        base_bash_libs_std_log_error -l base_bash_libs.std "base_bash_libs_std_wait_for_enter: invalid arguments"
        base_bash_libs_std_log_info -l base_bash_libs.std "Usage: base_bash_libs_std_wait_for_enter [prompt_message]"
        return 1
    fi

    local prompt=${1:-"Press Enter to continue"} tty_fd read_status
    if ! exec {tty_fd}</dev/tty 2>/dev/null; then
        base_bash_libs_std_log_error -l base_bash_libs.std "base_bash_libs_std_wait_for_enter: /dev/tty is not available"
        return 1
    fi
    ...
    if ((read_status != 0)); then
        base_bash_libs_std_log_error -l base_bash_libs.std "base_bash_libs_std_wait_for_enter: failed to read from /dev/tty"
        return "$read_status"
    fi
