...
- `bl_file_section_exists <target> <start_marker> <end_marker>`
...
- `bl_file_section_needs_update <target> <start_marker> <end_marker> [content...]`
...
- `bl_file_update_file_section [-r] <target> <start_marker> <end_marker> [content...]`
...
bl_file_update_file_section ~/.bash_profile "# BEGIN APP" "# END APP" \
...
```bash
if bl_file_section_needs_update ~/.bash_profile "# BEGIN APP" "# END APP" \
...
- `bl_file_section_exists` returns `0` when a valid marker pair is present, `1`
  when the target file is missing or the section is absent, and `2` when marker
  pairs are asymmetric or misordered.
- `bl_file_section_needs_update` returns `0` when an add/update would change the
