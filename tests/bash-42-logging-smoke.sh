...
main() {
    ...
    local -a smoke_args=()
    if ! bl_init smoke_args --source "${BASH_SOURCE[0]}" --; then
        smoke_fail "unable to initialize lib_std.sh."
        return 1
    fi

    if ! bl_std_check_bash_version; then
        smoke_fail "the running Bash did not satisfy the supported version check."
        return 1
    fi

    bl_std_make_temp_dir smoke_dir bash42-logging-smoke || {
    ...
    smoke_capture_stderr "$terminal_stdout" "$terminal_stderr" "default INFO" \
        bl_std_log_info -l base_bash_libs.smoke "default info" || return 1
    ...
    smoke_capture_stderr "$terminal_stdout" "$terminal_stderr" "default DEBUG" \
        bl_std_log_debug -l base_bash_libs.smoke "hidden library debug" || return 1
    debug_output="$SMOKE_CAPTURED_STDERR"
    if [[ -n "$debug_output" ]]; then
        smoke_fail "library DEBUG was visible at the default thresholds."
        return 1
    fi
    if bl_std_log_is_enabled -l base_bash_libs.smoke DEBUG; then
        smoke_fail "bl_std_log_is_enabled accepted library DEBUG at the default thresholds."
        return 1
    fi
    ...
    bl_std_set_log_category_level -l base_bash_libs.smoke DEBUG || {
        smoke_fail "unable to enable the smoke DEBUG category."
        return 1
    }
    if ! bl_std_log_is_enabled -l base_bash_libs.smoke.child DEBUG; then
    ...
    smoke_capture_stderr "$terminal_stdout" "$terminal_stderr" "enabled DEBUG" \
        bl_std_log_debug -l base_bash_libs.smoke.child "visible category debug" || return 1
    debug_output="$SMOKE_CAPTURED_STDERR"
    ...
    if [[ -e "$primary_log" ]]; then
        smoke_fail "bl_std_log_is_enabled modified the primary sink."
        return 1
    fi

    smoke_capture_stderr "$terminal_stdout" "$terminal_stderr" "persistent DEBUG" \
        bl_std_log_debug -l base_bash_libs.smoke.child "persisted debug" || return 1
    ...
    bl_std_set_log_category_level -l base_bash_libs.smoke INFO || {
    ...
    smoke_capture_stderr "$terminal_stdout" "$terminal_stderr" "blocked persistent DEBUG" \
        bl_std_log_debug -l base_bash_libs.smoke.child "blocked persistent debug" || return 1
    ...
    smoke_capture_stderr "$terminal_stdout" "$terminal_stderr" "persistent DEBUG file" \
        bl_std_log_debug_file -l base_bash_libs.smoke.child "$payload_file" || return 1
    terminal_output="$SMOKE_CAPTURED_STDERR"
    if [[ -n "$terminal_output" ]]; then
    ...
    smoke_capture_stderr "$terminal_stdout" "$terminal_stderr" "UTC INFO" \
        bl_std_log_info -l base_bash_libs.smoke "utc info" || return 1
    ...
    bl_std_set_log_category_level -l base_bash_libs.smoke VERBOSE || {
    ...
    smoke_capture_stderr "$terminal_stdout" "$terminal_stderr" "VERBOSE compatibility" \
        bl_std_log_verbose -l base_bash_libs.smoke "compat verbose" || return 1
